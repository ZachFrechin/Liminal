<?php

declare(strict_types=1);

// Contributed after the rendering lib's catalogue, so these keys win any
// collision — the same last-wins layering the container applies.
return [
    'authentication.login.title' => 'Sign in',
    'authentication.login.email' => 'Email',
    'authentication.login.password' => 'Password',
    'authentication.login.submit' => 'Sign in',
    // Deliberately identical for an unknown email and a wrong password: telling
    // the two apart is an account-enumeration oracle.
    'authentication.login.refused' => 'Those credentials were not accepted.',
    'authentication.login.throttled' => 'Too many attempts. Please wait before trying again.',
    'authentication.login.welcome' => 'Welcome back.',

    'authentication.account.title' => 'Your account',
    'authentication.account.signed_in_as' => 'Signed in as %name% (%email%).',

    'authentication.token.section' => 'API tokens',
    'authentication.token.section_hint' => 'A token authenticates as you over the JSON API — it grants nothing you do not already hold.',
    'authentication.token.label' => 'Label',
    'authentication.token.created' => 'Created',
    'authentication.token.last_used' => 'Last used',
    'authentication.token.never_used' => 'never',
    'authentication.token.create' => 'Create a token',
    'authentication.token.revoke' => 'Revoke',
    'authentication.token.revoked' => 'Token revoked.',
    'authentication.token.unknown' => 'No such token on this account.',
    'authentication.token.created_title' => 'API token created',
    'authentication.token.created_for' => 'The token "%label%" is ready:',
    'authentication.token.shown_once' => 'Copy it now — only its hash is stored, it will never be shown again.',
    'authentication.token.back_to_account' => 'Back to the account',
    'authentication.account.companies' => 'Companies you can reach',
    'authentication.account.current' => 'current',
    'authentication.account.switch' => 'Work in this company',
    'authentication.account.sign_out' => 'Sign out',

    'authentication.switch.done' => 'Working company switched.',
    'authentication.switch.refused' => 'That company is not yours to work in.',

    'authentication.logout.done' => 'You have been signed out.',

    'authentication.users.title' => 'Users',
    'authentication.users.create' => 'Create a user',
    'authentication.users.email' => 'Email',
    'authentication.users.name' => 'Name',
    'authentication.users.roles' => 'Roles in this company',
    'authentication.users.status' => 'Status',
    'authentication.users.inactive' => 'inactive',
    // Zero grants anywhere = zero accessible companies = cannot sign in.
    'authentication.users.no_access' => 'no access',

    'authentication.user.title' => 'User',
    'authentication.user.back' => 'Back to users',
    'authentication.user.create_title' => 'Create a user',
    'authentication.user.create_submit' => 'Create',
    'authentication.user.email' => 'Email',
    'authentication.user.display_name' => 'Display name',
    'authentication.user.invalid_email' => 'That email address is not valid.',
    'authentication.user.email_taken' => 'A user with that email already exists.',
    'authentication.user.name_required' => 'The display name cannot be empty.',
    'authentication.user.created_title' => 'User created',
    'authentication.user.password_for' => 'The initial password for %email% is:',
    'authentication.user.password_once' => 'Copy it now — it is shown only this once and never stored in the clear.',
    'authentication.user.back_to_user' => 'Go to the user',
    'authentication.user.active' => 'Active',
    'authentication.user.save' => 'Save',
    'authentication.user.updated' => 'User updated.',
    'authentication.user.self_note' => 'You cannot deactivate your own account.',
    'authentication.user.self_deactivation' => 'You cannot deactivate your own account.',
    'authentication.user.self_deletion' => 'You cannot delete your own account.',
    'authentication.user.deleted' => 'User deleted.',
    'authentication.user.grants' => 'Roles per company',
    'authentication.user.grant_company' => 'Company',
    'authentication.user.grant_role' => 'Role',
    'authentication.user.no_grants' => 'No role in any company — this user cannot sign in.',
    'authentication.user.add_grant' => 'Grant this role',
    'authentication.user.revoke' => 'Revoke',
    'authentication.user.grant_added' => 'Role granted.',
    'authentication.user.grant_already' => 'That role was already granted in that company.',
    'authentication.user.grant_removed' => 'Role revoked.',
    'authentication.user.grant_missing' => 'There was no such grant to revoke.',
    'authentication.user.grant_invalid' => 'That company or role does not exist.',
    'authentication.user.password' => 'Password',
    'authentication.user.reset_password' => 'Generate a new password',
    'authentication.user.password_reset_title' => 'Password reset',
    'authentication.user.danger' => 'Danger zone',
    'authentication.user.delete' => 'Delete this user',

    'authentication.roles.title' => 'Roles',
    'authentication.roles.create' => 'Create a role',
    'authentication.roles.code' => 'Code',
    'authentication.roles.label' => 'Label',
    'authentication.roles.permissions' => 'Permissions',
    'authentication.roles.grants' => 'Holders (all companies)',

    'authentication.role.title' => 'Role',
    'authentication.role.create_title' => 'Create a role',
    'authentication.role.create_submit' => 'Create',
    'authentication.role.code' => 'Code',
    'authentication.role.code_hint' => 'Lowercase letters, digits and underscores; the code never changes afterwards.',
    'authentication.role.back' => 'Back to roles',
    'authentication.role.label' => 'Label',
    'authentication.role.permissions' => 'Permissions',
    'authentication.role.invalid' => 'The role needs a valid code and a label.',
    'authentication.role.code_taken' => 'A role with that code already exists.',
    'authentication.role.created' => 'Role created.',
    'authentication.role.updated' => 'Role updated. Holders see the change on their next request.',
    'authentication.role.deleted' => 'Role deleted, and every grant of it revoked.',
    'authentication.role.unknown' => 'Held but no longer declared by any module (inert; removed at the next save):',
    'authentication.role.save' => 'Save',
    'authentication.role.danger' => 'Danger zone',
    'authentication.role.delete' => 'Delete this role',
    'authentication.role.delete_warning' => 'Deleting a role revokes it from every holder in every company at once.',
    'authentication.role.admin_protected' => 'The admin role cannot be deleted.',
    'authentication.role.admin_note' => 'This role is the bootstrap anchor: its permissions are editable, the role itself cannot be deleted.',

    'authentication.menu.account' => 'Account',
    'authentication.menu.users' => 'Users',
    'authentication.menu.roles' => 'Roles',

    'authentication.permission.user.manage' => 'Manage users',
    'authentication.permission.role.manage' => 'Manage roles',
];
