...
/**
 * One order line, parsed and validated. The caps are load-bearing, not
 ...
 */
final readonly class OrderLineForm
{
    ...
    public static function fromBody(array $body): self
    {
        ...
        if ($label === '' || mb_strlen($label) > 255) {
            $errors[] = 'order.form.label_required';
        }

        $quantity = self::amount($body['quantity'] ?? null, self::QUANTITY, 'order.form.quantity_invalid', $errors);
        $unitPrice = self::amount($body['unit_price'] ?? null, self::UNIT_PRICE, 'order.form.unit_price_invalid', $errors);
        $vatRate = self::amount($body['vat_rate'] ?? null, self::VAT_RATE, 'order.form.vat_rate_invalid', $errors);

        if ($vatRate !== '' && (float) $vatRate > 100.0) {
            // Comparison only — the stored value stays the validated string.
            $errors[] = 'order.form.vat_rate_invalid';
        }
