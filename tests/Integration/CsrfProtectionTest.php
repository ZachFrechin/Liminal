...
/**
 * The synchronizer token through real HTTP: unsafe methods refuse without it,
 * both transports work, and login rotates it.
 */
#[CoversNothing]
final class CsrfProtectionTest extends IntegrationTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if ($dsn === null) {
            self::markTestSkipped('LIMINAL_TEST_DSN is not set; skipping CSRF test.');
        }
    ...
    public function testAMutatingRequestWithoutATokenIs403(): void
    {
        $response = $this->kernel()->handle(
            $this->post('/login', ['identifier' => 'alice', 'password' => 'alice-secret']),
        );
    ...
    public function testTheTokenPassesInBodyOrHeader(): void
    {
        $kernel = $this->kernel();
        [$cookie, $token] = $this->obtainToken($kernel);

        $viaBody = $kernel->handle($this->post(
            '/login',
            ['identifier' => 'alice', 'password' => 'alice-secret', '_token' => $token],
            $cookie,
        ));

        self::assertSame(200, $viaBody->getStatusCode());

        [$cookie2, $token2] = $this->obtainToken($kernel);

        $viaHeader = $kernel->handle(
            $this->post('/login', ['identifier' => 'alice', 'password' => 'alice-secret'], $cookie2)
                ->withHeader('X-CSRF-Token', $token2),
        );

        self::assertSame(200, $viaHeader->getStatusCode());
    }

    public function testAForeignSessionsTokenIsRefused(): void
    {
        $kernel = $this->kernel();
        [, $foreignToken] = $this->obtainToken($kernel);
        [$cookie] = $this->obtainToken($kernel);

        $response = $kernel->handle($this->post(
            '/login',
            ['identifier' => 'alice', 'password' => 'alice-secret', '_token' => $foreignToken],
            $cookie,
        ));
    ...
    {
        $kernel = $this->kernel();
        [$cookie, $before] = $this->obtainToken($kernel);

        $login = $kernel->handle($this->post(
            '/login',
            ['identifier' => 'alice', 'password' => 'alice-secret', '_token' => $before],
            $cookie,
        ));
        $authenticated = $this->cookieValue($login);

        [, $after] = $this->obtainToken($kernel, $authenticated);
